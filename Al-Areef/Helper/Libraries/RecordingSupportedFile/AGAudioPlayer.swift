...
class AGAudioPlayer: NSObject {
    ...
    func preparePlay() {
        ...
        do {
            ...
            UserDefaults.standard.setValue( fileManager.fileUrl().lastPathComponent, forKey: "URL")
    ...
    func doPlay() {
        ...
        player.play()
        self.scheduleTimer(progrssBar: UIProgressView())
    ...
    func doStop() {
     ...
     func scheduleTimer( progrssBar : UIProgressView) {
        ...
        timer = Timer(timeInterval: updateInterval, repeats: true, block: { [weak self] _ in
            ...
            print(progress)
            progrssBar.progress  = Float(progress)
            progrssBar.setProgress(Float(progress), animated: true)
        })
