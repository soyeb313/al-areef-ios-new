...
protocol SendTimer {
    func sendttimer(intreval:TimeInterval, time:String)
}
class AGAudioRecorder: NSObject {
   ...
   open var delegetTimer : SendTimer? = nil
    
  public   var recorder: AVAudioRecorder? = nil
    ...
    @objc private func updateAudioMeter(timer: Timer) {
        ...
        if recorder.isRecording {
            ...
            delegetTimer?.sendttimer(intreval: recorder.currentTime, time: String(format: "%02d:%02d:%02d", hr, min, sec))
        } else {
            doStop()
    ...
    func doRecord() {
        ...
        do {
            ...
            recorder.record(forDuration: TimeInterval(duration1))
